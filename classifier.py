...
class Classifier:
    ...
    def train(self, files, gt_pos, durations, classes):
        """
        ...
        self.model.train(positions, class_labels, files, durations, classes)
    ...
    def test_single(self, audio_samples, sampling_rate):
        ...
        nms_pos, nms_prob, y_prediction, matches  = self.model.test(file_duration=duration, audio_samples=audio_samples, sampling_rate=sampling_rate)
        return nms_pos, nms_prob, y_prediction, matches
    ...
    def test_batch(self, files, gt_pos, durations, save_results=False, op_im_dir=''):
        ...
        classes = [None]*len(files)
        for ii, file_name in enumerate(files):
            nms_pos[ii], nms_prob[ii], y_prediction, classes[ii] = self.model.test(file_name=file_name, file_duration=durations[ii])
        ...
        return nms_pos, nms_prob, classes
    ...
    def do_hnm(self, files, gt_pos, durations, positions, class_labels):
        ...
        nms_pos, nms_prob, classes = self.test_batch(files, gt_pos, durations, False, '')
